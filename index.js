...
function findElementGreaterThan(arr, num) {
  ...
  const bigFirstNum = arr.find((elem) => elem > num);
  // Перевіряємо чи є елемент який задовільняє умову
  return bigFirstNum !== undefined
    ? bigFirstNum
...
function findIndexByProperty(arr, name) {
  ...
  const founded = arr.findIndex((item) => item.name === name);
  // Повертаємо знайдений індекс або -1, якщо об'єкт не знайдено.
  return founded !== -1 ? founded : -1;
}
...
function findLastEvenNumber(arr) {
  ...
  const found = arr.findLast((elem) => elem % 2 === 0);
...
function getArrayKeys(arr) {
  ...
  const arrayKeys = Array.from(iteratorKeys);
...
function getPositiveArrayValues(arr) {
  ...
  const getVall = arr.values();
  // Конвертуємо ітератор в масив.
  const convertVall = Array.from(getVall);
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const positiveArrayValues = convertVall.filter((elem) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrayRemove = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const arrSquared = arrFlat.map((x) => x * x);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const filteredSquaredArray = arrSquared.filter((elem) => elem > threshold);
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((elem1, elem2) =>
    elem1.length > elem2.length ? elem1 : elem2
  );
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((num1, num2) =>
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((num, elem) => {
    return elem.price + num;
  }, 0);
